<script>
    ...
    void main() {
        ...
        float noiseSpeed = uTime * uNoiseSpeed * 0.1;
        
        float baseNoise = perlin(vec3(coord * noiseScale, noiseSpeed)) * 0.5 + 0.5;
        vec3 grain = vec3(whiteNoise(coord, grainScale, uTime * grainSpeed));
	...
	onMount(() => {
		glRenderer = setupGL(
			...
			vertexShader,
			fragmentShader,
			(gl, contextLost) => render(gl, canvas, contextLost, uniforms),
			fps
		);
